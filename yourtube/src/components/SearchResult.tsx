...
const SearchResult = ({ query }: any) => {
  ...
  const videos = async () => {
    ...
    let results = allVideos.filter(
      (vid) =>
        vid.videotitle.toLowerCase().includes(searchQuery.toLowerCase()) ||
        vid.videochanel.toLowerCase().includes(searchQuery.toLowerCase())
    );
    setvideos(results);
  };
  useEffect(() => {
    ...
    <div className="space-y-6">
      ...
      {hasResults && (
        <div className="text-center py-8">
          <p className="text-gray-600">
            Showing {video.length} results for "{searchQuery}"
          </p>
        </div>
      )}
    </div>
